  ...
  methods: {
    async searchProblems() {
      ...
      try {
        ...
        const res = await searchLeetCodeProblems(params);
    ...
    async selectProblem(problem) {
      try {
        const detail = await getLeetCodeProblemDetail(problem.id);
        this.selectedProblem = detail;
    ...
    async getHint() {
      ...
      try {
        const res = await getLeetCodeHint(this.selectedProblem.id, this.selectedLanguage, this.userCode);
        this.currentHint = res.hint || '暂无AI提示';
      } catch (error) {
        alert('获取提示失败，请重试');
      } finally {
    ...
    async runCode() {
      ...
      this.isRunning = true;
      try {
        const res = await runLeetCodeCode(this.selectedProblem.id, this.selectedLanguage, this.userCode);
        this.runResult = res;
      } catch (error) {
        this.runResult = { success: false, error: '运行失败，请检查代码或稍后重试' };
      } finally {
    ...
    async submitCode() {
      ...
      try {
        const res = await submitLeetCodeCode(this.selectedProblem.id, this.selectedLanguage, this.userCode);
        if (res.success) {
          alert('🎉 恭喜！代码提交成功，所有测试用例通过！');
        } else {
          alert(res.message || '❌ 提交失败，部分测试用例未通过，请检查代码逻辑。');
        }
      } catch (error) {
        alert('提交代码失败，请重试');
      } finally {
